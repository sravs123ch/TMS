          ...
          message={
            <div className="space-y-4">
              <p className="text-gray-700 text-sm">
                Please provide a reason for updating the plant assignments for
                user "<strong>{selectedPlantAssignment?.fullName}</strong>" from
                "
                <strong>
                  {initialFormDataRef.current?.selectedPlant
                    .map((id) => plants.find((p) => p.value === id)?.label)
                    .filter(Boolean)
                    .join(", ")}
                </strong>
                " to "
