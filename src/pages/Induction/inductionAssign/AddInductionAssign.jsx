...
import {CustomAsyncSelect,EnhancedInputField} from "../../../components/common/ui/FormFields";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.header?.errorCount === 0) {
        toast.success(
          response.header?.messages?.[0]?.messageText || "Assigned successfully."
        );
              ...
              value={
                users.find((u) => u.userID === formData.userID)
                  ? {
                      ...
                      label: users.find((u) => u.userID === formData.userID).userName,
                    }
                  : null
              }
              onChange={(selected) =>
